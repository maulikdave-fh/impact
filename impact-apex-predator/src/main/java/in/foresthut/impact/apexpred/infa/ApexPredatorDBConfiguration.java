...
public class ApexPredatorDBConfiguration {
	private static ApexPredatorDBConfiguration instance;
	private MongoClient mongoClient;
	private MongoDatabase tropicWebDB;
	private static final Logger logger = LoggerFactory.getLogger(ApexPredatorDBConfiguration.class);
	private static Config config = Config.getInstance();

	private ApexPredatorDBConfiguration() {
		...
		tropicWebDB = mongoClient.getDatabase(config.get("mongodb.database"));
		try {
			// Send a ping to confirm a successful connection
			Bson command = new BsonDocument("ping", new BsonInt64(1));
			Document commandResult = tropicWebDB.runCommand(command);
	...
	public static synchronized ApexPredatorDBConfiguration getInstance() {
		if (instance == null)
			instance = new ApexPredatorDBConfiguration();
		return instance;
	}	
