...
public class IntroducedUtility {
	private static IntroducedUtility instance;
	private static final Logger logger = LoggerFactory.getLogger(IntroducedUtility.class);
	...
	private IntroducedUtility() {

	}

	public static IntroducedUtility getInstance() {
		if (instance == null)
			instance = new IntroducedUtility();
		return instance;
	}

	public boolean isIntroduced(String species, String region) {
		if (bloomFilters.containsKey(region)) {
			...
			if (bloomFilter.mightContain(species)) {
				...
				return isIntroducedInDB(species, region);
			} else {
				logger.info("{} didn't find in the BloomFilter. It is not an introduced species in {}", species, region);
				return false;
			}
		} else {
			...
			var introducedSpeciesCollection = speciesAttributesDB.getCollection("introduced_species");
	...
	private boolean isIntroducedInDB(String species, String region) {
		var introducedSpeciesCollection = speciesAttributesDB.getCollection("introduced-species");

		Bson filter = Filters.eq("name", species);
		long recordCount = introducedSpeciesCollection.countDocuments(filter);
		if (recordCount > 0)
			logger.info("{} found in DB. It is an introduced species in {}.", species, region);
		else
			logger.info("{} not found in DB. It is not an introduced species in {}.", species, region);
