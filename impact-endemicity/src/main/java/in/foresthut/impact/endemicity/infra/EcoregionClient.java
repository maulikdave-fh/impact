...
public class EcoregionClient {
	...
	public String regionMap(String regionName) {
		String ECOREGIONS_URL = new String();
		try {
			ECOREGIONS_URL = ECOREGION_HOST + "/region/" + URLEncoder.encode(regionName, "UTF-8") + "/combine";
		} catch (UnsupportedEncodingException e) {
			logger.error("Encoding not supported for URL {}", ECOREGIONS_URL, e);
			throw new AlreadyLoggedException("Encoding not supported for URL " + ECOREGIONS_URL, null, e);
		}
		HttpRequest request = HttpRequest.newBuilder().uri(URI.create(ECOREGIONS_URL)).GET().build();
		try {
			HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
			String regionMap = objectMapper.readValue(response.body(), String.class);
			return regionMap;
		} catch (Exception ex) {
			logger.error("Error while fetching region map for {}", regionName, ex);
			throw new AlreadyLoggedException("Error while fetching ecoregions", null, ex);
		}
	}
}
